...
int DMP_Init(void)
{
  ...
  da[0] = 0x80; if(I2C_Write(0x6B, 1, &(da[0])))	return 1;	// 复位
  system_delay_ms(100);														// 延迟
  da[0] = 0x00; if(I2C_Write(0x6B, 1, &(da[0])))	return 2;	// 唤醒6050
  
  if(I2C_Read( 0x06, 6, da)) return 3;							// 读0x06寄存器
  
  rev = ((da[5] & 0x01) << 2) | ((da[3] & 0x01) << 1) | (da[1] & 0x01); // 读时序得到的值一直是固定的，rev等于0x10
  ...
  da[0] = 0x00; if(I2C_Write(0x38, 1, &(da[0]))) return 10;	// 这之后st.chip_cfg.int_enable = 0;
  
  // mpu_set_bypass(0);
  da[0] = 0x00; if(I2C_Read( 0x6A, 1, &(da[0]))) 	return 11;
  da[0] = 0x20; if(I2C_Write(0x6A, 1, &(da[0]))) return 12;
  system_delay_ms(3);
  da[0] = 0x80; if(I2C_Write(0x37, 1, &(da[0]))) return 13;	// 这之后st.chip_cfg.bypass_mode = 0;
  
  //mpu_set_sensors(0);
  da[0] = 0x40; if(I2C_Write(0x6B, 1, &(da[0]))) return 14; 	// 这之后st.chip_cfg.clk_src = 0;
  da[0] = 0x3F; if(I2C_Write(0x6C, 1, &(da[0]))) return 15; 	// 这之后st.chip_cfg.sensors = 0;
  //      st.chip_cfg.lp_accel_mode = 0;
  system_delay_ms(50);
  // 结束mpu_init()
  
  // mpu_set_sensors(0x70 | 0x08)  
  da[0] = 0x01; if(I2C_Write(0x6B, 1,  &(da[0]))) return 16; // 这之后st.chip_cfg.clk_src = 0x01;
  
  da[0] = 0x00; if(I2C_Write(0x6C, 1,  &(da[0]))) return 17;
  ...
  // 其中set_int_enable(1);
  da[0] = 0x01; if(I2C_Write(0x38, 1, &(da[0]))) return 18;	// 在这之后st.chip_cfg.int_enable = 0x01;
  // 其中mpu_reset_fifo(void)
  da[0] = 0x00; if(I2C_Write(0x38, 1, &(da[0]))) return 19;
  da[0] = 0x00; if(I2C_Write(0x23, 1, &(da[0]))) return 20;
  da[0] = 0x00; if(I2C_Write(0x6A, 1, &(da[0]))) return 21;
  da[0] = 0x04; if(I2C_Write(0x6A, 1, &(da[0]))) return 22;
  da[0] = 0x40; if(I2C_Write(0x6A, 1, &(da[0]))) return 23;
  da[0] = 0x01; if(I2C_Write(0x38, 1, &(da[0]))) return 24;
  da[0] = 0x78; if(I2C_Write(0x23, 1, &(da[0]))) return 25;
  // 结束
  ...
  tmp[0] = 0x04; tmp[1] = 0x00; if(I2C_Write(0x70, 2, tmp)) return -1;	// 这之后st.chip_cfg.dmp_loaded = 1;
  ...
  // mpu_set_dmp_state(1)
  /* Disable da ready interrupt. */
  //    set_int_enable(0);
  tmp[0] = 0x00;
  if (I2C_Write(0x38, 1, tmp)) return -1; 					// 在这之后st.chip_cfg.int_enable = 0;
  ...
  tmp[0] = 0x00;
  if (I2C_Write(0x23, 1, tmp)) return -1;					// 在这之后st.chip_cfg.dmp_on = 1;
  /* Enable DMP interrupt. */
  //    set_int_enable(1); 
  tmp[0] = 0x02;
  if (I2C_Write(0x38, 1, tmp)) return -1;					// 在这之后st.chip_cfg.int_enable = 0x02;
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
  ...
  switch(fsr) 
  {
  case 250: da = 0 << 3; break;
  case 500: da = 1 << 3; break;
  case 1000: da = 2 << 3; break;
  case 2000: da = 3 << 3; break;
  default: return -1;
  }
  if(I2C_Write(0x1B, 1, &da)) return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
  ...
  case 4: da = 1 << 3; break;
  case 8: da = 2 << 3; break;
  case 16: da = 3 << 3; break;
  default : return -1;
  }
  
  if (I2C_Write(0x1C, 1, &da)) return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
  ...
  if (I2C_Write(0x1A, 1, &da)) return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
  ...
  da = 1000 / rate - 1;
  if(I2C_Write(0x19, 1, &da)) return -1;
...
int mpu_write_mem(unsigned short mem_addr, unsigned short length, unsigned char *da)
{
  ...
  if (I2C_Write(0x6D, 2, tmp))
    return -1;
  if (I2C_Write(0x6F, length, da))
    return -1;
  return 0;
}
// mpu读函数，mem_addr一般为16位，先从高到低发送16位地址到0x6D寄存器，再从0x6F寄存器读取length长度的数据到da
int mpu_read_mem(unsigned short mem_addr, unsigned short length, unsigned char *da)
{
  ...
  if (I2C_Write(0x6D, 2, tmp)) return -1;
  if (I2C_Read( 0x6F, length, da)) return -1;
  return 0;
}
...
int mpu_reset_fifo_in_dmp_enable_feature(void)
{
  ...
  da = 0;
  if (I2C_Write(0x38, 1, &da)) return -1;
  if (I2C_Write(0x23, 1, &da)) return -1;
  if (I2C_Write(0x6A, 1, &da)) return -1;
  da = 0x04;
  if (I2C_Write(0x6A, 1, &da)) return -1;
  da = 0x40;
  if (I2C_Write(0x6A, 1, &da)) return -1;
...
int DMP_Read(void)
{
    ...
    if (sensors & 0x100 )
    {
        ...
        Pitch =  asin(-2 * q1 * q3 + 2 * q0* q2)* 57.296;// Pitch
        Roll  = -atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.296; // roll
...
int dmp_read_fifo(short *gyro, short *accel, long *quat, short *sensors, unsigned char *more)
{
  ...
  if (I2C_Read( 0x72, 2, tmp)) return -1;
  ...
  if (fifo_count > 512) 
  {
    ...
    // int_status = 0x3A
    if (I2C_Read( 0x3A, 1, tmp)) return -1;
  ...
  if (I2C_Read( 0x74, 32, fifo_data)) return -1;
  ...
  ii += 16;
  /* We can detect a corrupted FIFO by monitoring the quaternion da and
  * ensuring that the magnitude is always normalized to one. This
  * shouldn't happen in normal operation, but if an I2C error occurs,
  * the FIFO reads might become misaligned.
  *
  * Let's start by scaling down the quaternion da to avoid double
  * math.
  */
  ...
  /* Gesture da is at the end of the DMP packet. Parse it and call
